<link href="<?php echo '/assets/bootstrap/css/bootstrap.min.css'; ?>" rel="stylesheet">

<?php require $header ?></body>

<div id="wrapper">
    <section class="bg-secondary">
        <div class="container py-4 py-xl-5">
            <div class="row gy-4 gy-md-0">
                <div
                        class="col-md-6 text-center text-md-start d-flex d-sm-flex d-md-flex justify-content-center align-items-center justify-content-md-start align-items-md-center justify-content-xl-center"
                >
                    <div>
                        <h1 class="text-info">Application de suivi des cryptomonnaies</h1>
                        <div>
                            <p class="text-info my-3 me-5">
                                Créer un compte gratuitement et visualisez facilement et rapidement l'évolution des
                                cryptomonnaies
                            </p>
                        </div>
                        <a
                                class="btn btn-primary btn-lg link-light border rounded-pill me-2"
                                role="button"
                                href="/register"
                        >
                            Créer un compte
                        </a>
                        <a
                                class="btn btn-outline-primary btn-lg link-light text-bg-primary border rounded-pill"
                                role="button"
                                href="/#a-propos"
                        >
                            En savoir plus
                        </a>
                    </div>
                    <div
                            style="max-width: 350px; padding-right: 0px; margin-top: 0px;"
                    ></div>
                </div>
                <div class="col-md-6">
                    <div class="p-xl-5 m-xl-5">
                        <div class="card border-0 mt-5">
                            <div
                                    class="card-body bg-secondary d-flex flex-column align-items-center"
                            >
                                <form class="text-center" method="post" action="/login">
                                    <div class="form-group mb-3">
                                        <label class="form-label float-start">
                                            E-mail
                                        </label>
                                        <input
                                                class="border form-control form-control-lg"
                                                type="email"
                                                name="email1"
                                                placeholder="Email"
                                        />
                                    </div>
                                    <div class="form-group mb-3">
                                        <label class="form-label float-start">Mot de passe</label>
                                        <input
                                                class="border form-control form-control-lg"
                                                type="password"
                                                name="password1"
                                                placeholder="Password"
                                        />
                                    </div>
                                    <div class="row mb-3">
                                        <?php if (isset($login_message)) : ?>
                                            <p class="text-danger"><?= $login_message ?></p>
                                        <?php endif ?>
                                    </div>
                                    <div class="mb-3">
                                        <button
                                                class="btn btn-primary btn-lg link-light text-bg-primary border rounded-pill d-block w-100"
                                                id="submitBtn1"
                                                type="submit"
                                        >
                                            Se connecter
                                        </button>
                                    </div>
                                    <!--                                <a class="text-decoration-none" href="#">-->
                                    <!--                                    <p class="text-muted">Mot de passe oublié?</p>-->
                                    <!--                                </a>-->
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="svg-border-rounded text-gray-dark">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 144.54 17.34" preserveAspectRatio="none"
                 fill="currentColor">
                <path d="M144.54,17.34H0V0H144.54ZM0,0S32.36,17.34,72.27,17.34,144.54,0,144.54,0"></path>
            </svg>
        </div><!----><!----><!---->
    </section>
    <section class="bg-dark py-10" id="a-propos">
        <div class="container py-4 py-xl-5">
            <div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3">
                <div class="col">
                    <div
                            class="text-center d-flex flex-column align-items-center align-items-xl-center"
                    >
                        <div
                                class="bs-icon-lg bs-icon-rounded bs-icon-primary d-flex flex-shrink-0 justify-content-center align-items-center d-inline-block mb-3 bs-icon lg"
                        >
                            <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="1em"
                                    height="1em"
                                    fill="currentColor"
                                    viewBox="0 0 16 16"
                                    class="bi bi-check"
                            >
                                <path
                                        d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"
                                ></path>
                            </svg>
                        </div>
                        <div class="px-3">
                            <p class="text-white">
                                Votre cryptomonnaie dans un graphique avec son cours et sa variation sur 24h
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div
                            class="text-center d-flex flex-column align-items-center align-items-xl-center"
                    >
                        <div
                                class="bs-icon-lg bs-icon-rounded bs-icon-primary d-flex flex-shrink-0 justify-content-center align-items-center d-inline-block mb-3 bs-icon lg"
                        >
                            <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="1em"
                                    height="1em"
                                    fill="currentColor"
                                    viewBox="0 0 16 16"
                                    class="bi bi-bar-chart"
                            >
                                <path
                                        d="M4 11H2v3h2v-3zm5-4H7v7h2V7zm5-5v12h-2V2h2zm-2-1a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1h-2zM6 7a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7zm-5 4a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1v-3z"
                                ></path>
                            </svg>
                        </div>
                        <div class="px-3">
                            <p class="text-white">
                                Votre cryptomonnaie dans un graphique représentant le "trading view"
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div
                            class="text-center d-flex flex-column align-items-center align-items-xl-center"
                    >
                        <div
                                class="bs-icon-lg bs-icon-rounded bs-icon-primary d-flex flex-shrink-0 justify-content-center align-items-center d-inline-block mb-3 bs-icon lg"
                        >
                            <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="1em"
                                    height="1em"
                                    fill="currentColor"
                                    viewBox="0 0 16 16"
                                    class="bi bi-star"
                            >
                                <path
                                        d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"
                                ></path>
                            </svg>
                        </div>
                        <div class="px-3">
                            <p class="text-white">
                                Votre cryptomonnaie avec le taux de change estimé
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="bg-dark py-10">
        <div class="container py-4 py-xl-5">
            <div class="row row-cols-1 row-cols-md-2">
                <div class="col d-flex flex-column justify-content-center p-4">
                    <div
                            class="text-center text-md-start d-flex flex-column align-items-center align-items-md-start mb-5"
                    >
                        <div>
                            <h2 class="text-white h1">Personnalisé votre application</h2>
                            <p class="text-white">
                                Votre application web et mobile est gratuite et personnalisée. En effet, vous pourrez
                                vous
                                connecter avec propre profil et ainsi pouvoir mettre en favoris les cryptomonnaies qui
                                vos
                                intéressent le plus. Cela vous permettra de ne pas être perdu dans un flot de données
                                qui ne
                                vous intéresse pas.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <img
                            class="rounded border rounded-0 w-100 h-100 fit-cover"
                            style="min-height: 300px;"
                            src="https://cdn.bootstrapstudio.io/placeholders/1400x800.png"
                    />
                </div>
            </div>
        </div>
    </section>
    <section class="bg-dark">
        <div class="container py-4 py-xl-5">
            <div class="row mb-5">
                <div class="col-md-8 col-xl-6 text-center mx-auto">
                    <h2 class="text-white">Télécharger l'application mobile</h2>
                    <p class="text-white w-lg-50">
                        Curae hendrerit donec commodo hendrerit egestas tempus, turpis
                        facilisis nostra nunc. Vestibulum dui eget ultrices.
                    </p>
                </div>
            </div>
            <div class="row row-cols-1 row-cols-md-2 row-cols-xl-2">
                <div class="col">
                    <div class="d-flex float-lg-end p-3">
                        <button class="btn btn-primary btn-lg" type="button">
                            <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="1em"
                                    height="1em"
                                    fill="currentColor"
                                    viewBox="0 0 16 16"
                                    class="bi bi-facebook"
                            >
                                <path
                                        d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"
                                ></path>
                            </svg>
                            Button
                            <span></span>
                        </button>
                    </div>
                </div>
                <div class="col">
                    <div class="d-flex float-lg-start p-3">
                        <button class="btn btn-primary btn-lg" type="button">
                            <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="1em"
                                    height="1em"
                                    fill="currentColor"
                                    viewBox="0 0 16 16"
                                    class="bi bi-facebook"
                            >
                                <path
                                        d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"
                                ></path>
                            </svg>
                            Button
                        </button>
                    </div>
                </div>
            </div>
        </div>
        <div class="svg-border-rounded text-light">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 144.54 17.34" preserveAspectRatio="none"
                 fill="currentColor">
                <path d="M144.54,17.34H0V0H144.54ZM0,0S32.36,17.34,72.27,17.34,144.54,0,144.54,0"></path>
            </svg>
        </div><!----><!----><!---->
    </section>
</div>

<?php require $footer ?></body>